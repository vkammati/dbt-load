...
with
rename_filter_kna1_stn as (

  select
  MANDT AS Client,
KUNNR AS Customer_Number,
ZBUYER_PRMRY_CNTRY_ID AS Buyer_Primary_Country_Id,
ZKUNNR_ID AS Customer_ID,
...
knb1_stn_delta as (
  select KNB1_Customer_Number from rename_filter_knb1_stn
  {% if is_incremental() %}
    where ingested_at>(select max(Ingested_at) from {{this}})
  {% endif %}
),
...
kna1_knb1_stn_pk as(
  select Customer_Number_delta from kna1_stn_delta
  union
  select KNB1_Customer_Number from knb1_stn_delta
),

kna1_stn_full(
  select rename_filter_kna1_stn.* except(ingested_at) from rename_filter_kna1_stn inner join kna1_knb1_stn_pk on
  rename_filter_kna1_stn.Customer_Number=kna1_knb1_stn_pk.Customer_Number_delta
),

kna1_knb1_stn_final(
  select kna1_stn_full.*,
  rename_filter_knb1_stn.* except (Client, KNB1_Customer_Number,ingested_at)
  from kna1_stn_full
  left join rename_filter_knb1_stn
  on kna1_stn_full.Customer_Number = rename_filter_knb1_stn.KNB1_Customer_Number
)

select kna1_knb1_stn_final.* except(Client),
current_timestamp() as Ingested_At,
('{{set_system_id()}}' || "_"|| kna1_knb1_stn_final.Client) as Source_ID
from kna1_knb1_stn_final
