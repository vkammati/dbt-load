...
lfb1_stn_delta as(
    ...
    from rename_filter_lfb1_stn
    {% if is_incremental() %}
     where ingested_at > (select max(Ingested_at) from {{this}})
    {% endif %}
),
lfa1_lfb1_stn_pk as(
    select Account_Number_Of_Supplier_delta from lfa1_stn_delta
    union
    select LFB1_Account_Number_Of_Supplier from lfb1_stn_delta
),
lfa1_stn_full(
    select rename_filter_lfa1_stn.* except (ingested_at) from rename_filter_lfa1_stn inner join lfa1_lfb1_stn_pk on
    rename_filter_lfa1_stn.Account_Number_Of_Supplier = lfa1_lfb1_stn_pk.Account_Number_Of_Supplier_delta
),
lfa1_lfb1_stn_final(
  select lfa1_stn_full.*,
  rename_filter_lfb1_stn.* except (Client,LFB1_Account_Number_Of_Supplier,ingested_at)
  from lfa1_stn_full
  left join rename_filter_lfb1_stn
  on lfa1_stn_full.Account_Number_Of_Supplier = rename_filter_lfb1_stn.LFB1_Account_Number_Of_Supplier
)

select lfa1_lfb1_stn_final.* except (Client),
current_timestamp() as Ingested_At,
('{{ set_system_id() }}' || "_" || lfa1_lfb1_stn_final.Client) as Source_ID
from lfa1_lfb1_stn_final
